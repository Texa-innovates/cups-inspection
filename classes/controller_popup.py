...
def _choose_backend():
    """
    Live preview open policy:
    - Prefer DEFAULT backend first (most compatible, like Code-1)
    - Windows: then try DSHOW as fallback
    - Linux: default is enough (V4L2 via default)
    """
...
# ==================================================
# CAMERA SCAN THREAD (Code-1 style)
# ==================================================
class CameraScanThread(QThread):
    """
    Fast scan like Code-1:
    - cv2.VideoCapture(index) default backend
    - check cap.isOpened()
    - release immediately
    Runs in background so UI animations stay smooth.
    """
    finished_scan = pyqtSignal(object)  # emits set(int)

    def __init__(self, max_index: int = 10, parent=None):
        super().__init__(parent)
    ...
    def run(self):
        ...
        for idx in range(self.max_index):
            ...
            cap = None
            try:
                cap = cv2.VideoCapture(idx)  # DEFAULT backend (most compatible)
...
# ==================================================
# CAMERA STATUS (UI animations, but scan is fast thread)
# ==================================================
class CameraStatusDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, max_index: int = 10):
        ...
        # UI reveal timer (NO camera open inside)
        self._timer = QtCore.QTimer(self)
        self._timer.timeout.connect(self._reveal_step)
    ...
    def start_scan(self):
        ...
        # animations while scan thread runs
        self._start_scan_anim()
        self._icon_anim.start()

        # start scan thread (Code-1 style detection)
        self._scan_thread = CameraScanThread(max_index=self.max_index, parent=self)
        self._scan_thread.finished_scan.connect(self._on_scan_done)
        self._scan_thread.start()

        # disable rescan while running
        self.btn_rescan.setEnabled(False)

    def _on_scan_done(self, connected_set):
        ...
        self._connected = set(int(x) for x in (connected_set or set()))
        self._scan_i = 0

        # now animate-reveal rows using cached results (no camera opens)
...
class Live4CamPopup(QtWidgets.QDialog):
    ...
    def __init__(self, cam_indexes=(0, 1, 2, 3), parent=None):
        super().__init__(parent)
