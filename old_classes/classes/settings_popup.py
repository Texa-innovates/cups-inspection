...
from classes.database import (create_database_and_tables,fetch_shifts, delete_shift_by_id, insert_shift,
...
class TickToggleSwitch(QtWidgets.QAbstractButton):
    ...
    def __init__(
        ...
        w: int = 90,
        h: int = 42,
        bg_off: QtGui.QColor = QtGui.QColor("#D8DEE9"),
        bg_on: QtGui.QColor = QtGui.QColor("#1F6FEB"),
        knob: QtGui.QColor = QtGui.QColor("#FFFFFF"),
        check_color: QtGui.QColor = QtGui.QColor("#1F6FEB"),
    ):
    ...
    def paintEvent(self, _):
        ...
        # check icon fade-in
        check_opacity = 0.10 + 0.90 * self._t
        p.setOpacity(check_opacity)

        pen = QtGui.QPen(self._check_color)
        pen.setWidthF(3.0)
        ...
        path.moveTo(cxk - 1.2 * s, cyk + 0.0 * s)
        path.lineTo(cxk - 0.3 * s, cyk + 0.9 * s)
        path.lineTo(cxk + 1.35 * s, cyk - 0.9 * s)
        p.drawPath(path)
