# =========================
# LIVE CAPTURE + REMBG (GPU) + SAVE BMP (NO PREDICTION)
# Folder: capture_remove/CAM1..CAM4
# Auto camera mapping (indices may change)
# camera capture = multi-threaded
# rembg processing = multi-threaded
# =========================

import os, sys, time, json, threading, platform, io, glob, subprocess, re
...
def scan_cameras(max_index: int = 50) -> List[dict]:
    """
    Scan /dev/video* and return working cameras.
    """
    cams = []
...
def resolve_cam_map(num_cams: int = 4) -> Dict[str, dict]:
    ...
    Returns mapping dict:
      {"CAM1": {...}, "CAM2": {...}, ...}
    Uses saved usb_id matching if possible; else rebuilds from scan order.
    """
...
# ================= rembg (GPU-only, thread-local sessions) ==================
_rebg_local = threading.local()

def get_thread_rembg_session():
    if not hasattr(_rebg_local, "session"):
        _rebg_local.session = new_session(
            BG_MODEL_NAME,
            providers=["CUDAExecutionProvider"]
        )
        print(f"[REMBG] Created session in thread {threading.get_ident()}")
    return _rebg_local.session

def remove_background_from_image(bgr_img: np.ndarray) -> np.ndarray:
    if bgr_img is None:
        raise ValueError("Invalid image for rembg")

    session = get_thread_rembg_session()
...
class CameraStream:
    ...
    def start(self):
        if os.name == "nt":
            ...
            backend = getattr(cv2, "CAP_V4L2", cv2.CAP_V4L)
        ...
        self.thread = threading.Thread(
            target=self._loop,
            daemon=True,
            name=f"CameraThread-{self.index}"
        )
        self.thread.start()

    def _loop(self):
        print(f"[CAM] Camera {self.index} capture thread started: {threading.get_ident()}")
...
class MultiCam:
    def __init__(self, cam_map: Dict[str, dict], w, h, fps):
        ...
        self.cams = {
            cam_key: CameraStream(info["index"], w, h, fps)
...
# ============== Worker helper ==============
def process_one_camera(cam_key: str, img: np.ndarray, real_idx: int, ts: str) -> Tuple[str, np.ndarray, str, str, bool]:
    """
    Returns:
        cam_key, img_bg_removed, save_path, log_line, ok
    """
    if img is None:
        raise RuntimeError(f"{cam_key} no frame (index={real_idx})")

    img_bg_removed = remove_background_from_image(img)
    ...
    ok = cv2.imwrite(save_path, img_bg_removed)
    if ok:
        log_line = f"[SAVE] {cam_key} (idx={real_idx}) -> {save_path}"
    else:
        log_line = f"[WARN] Failed to save: {save_path}"

    return cam_key, img_bg_removed, save_path, log_line, ok
...
class PLCWorker(QThread):
    ...
    def __init__(self, parent=None):
        ...
        self.mc.start_all()
        self.cup_count = 0
        self.rembg_pool = ThreadPoolExecutor(
            max_workers=min(REMBG_WORKERS, len(CAM_MAP)),
            thread_name_prefix="RembgWorker"
        )
    ...
    def run(self):
        try:
            ...
            self.sig_log.emit(f"[{now_str()}] ✅ MODE: AUTO CAM MAP + REMBG(GPU PARALLEL) + SAVE BMP (NO PREDICTION)")

            ordered_cam_keys = ["CAM1", "CAM2", "CAM3", "CAM4"]
            last = 0
            ...
            while self._running:
                ...
                if val == 1 and last != 1:
                    ...
                    futures = {}
                    for cam_key in ordered_cam_keys:
                        ...
                        fut = self.rembg_pool.submit(process_one_camera, cam_key, img, real_idx, ts)
                        futures[fut] = cam_key
                    ...
                    for fut in as_completed(futures):
                        cam_key = futures[fut]
                        try:
                            out_cam_key, img_bg_removed, save_path, log_line, ok = fut.result()
                            results_by_cam[out_cam_key] = (img_bg_removed, save_path, log_line, ok)
                        except Exception as e:
                            self.sig_log.emit(f"[ERROR] {cam_key} rembg/save failed: {e}")

                    for cam_key in ordered_cam_keys:
                        if cam_key not in results_by_cam:
                            continue
                        img_bg_removed, save_path, log_line, ok = results_by_cam[cam_key]
                        self.sig_log.emit(log_line)
                        self.sig_cam_vis.emit(cam_key, qimage_from_bgr(img_bg_removed))
            ...
            try:
                self.rembg_pool.shutdown(wait=True)
            except Exception:
                pass
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.title_lbl = QLabel("Capture + Remove BG + Save BMP (AUTO CAM MAP + PARALLEL REMBG)")
    ...
    def start_worker(self):
        ...
        self.append_log(f"[{now_str()}] ▶️ Start (auto camera mapping + parallel rembg)")
        self.worker.start()
